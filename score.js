...
var getIndiaMatch = function(url){
    request(url,(err,req,body)=>{
        ...
        if(body.state == 'mom' || body.state == 'complete'){
            ...
            clearInterval(gp);
        } else if (body.toss && showToss && !body.score){
            showToss = true;
            ...
            if(body.score.batsman.length > 1){
                ...
                if(body.score.batsman[0].id == bt1){
                    sendNotification(comm_lines[0].comm.replace('<b>','').replace('</b>','').replace("'","\\'"),'','Wicket',bt2.fn?bt2.fn : '');
                }
                else{
                    sendNotification(comm_lines[0].comm.replace('<b>','').replace('</b>','').replace("'","\\'"),'','Wicket',bt1.fn ? bt1.fn : "");
                }
                bt1 ={};
                bt2 ={};
            }
